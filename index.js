...
const colors = ['92ce14', '4bd588'];
function color () {
  colors.forEach(function (item, number) {
      setTimeout(function () {bott.guilds.get('443386238838374400').roles.get('451838923325308929').setColor(item).catch();if(number === colors.length-1) setTimeout(function () {color()}, 1000)}, number*1500);
  });
}
...
bott.on('guildMemberAdd', member => {
  bott.channels.get('452358352031186954').send(`Новый гражданин был зарегисрирован. Под именем ${member}! Первое что ты должен сделать, пойти к #джон, удачи!`)
});
